...
interface SemesterState {
  cp1: string; cp2: string; sprint1: string; sprint2: string; gs: string
}
      ...
      <Card className="border-t-4 border-t-primary shadow-lg">
        <CardHeader>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-primary">FIAP · Resultado</span>
          </div>
        ...
        <CardContent>
          <GradeResult result={result} targetGrade={Number(targetGrade) || 6.0} onReset={handleReset} />
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card className="border-t-4 border-t-primary shadow-lg">
      <CardHeader>
        <div className="flex items-center gap-2 mb-1">
          <GraduationCap className="h-4 w-4 text-primary" />
          <span className="text-[10px] font-bold uppercase tracking-widest text-primary">FIAP · Notas</span>
        </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-1">
          <Label htmlFor="target">Nota alvo <span className="text-muted-foreground font-normal">(opcional)</span></Label>
          <Input
            id="target"
            type="number"
            min={0.01}
          ...
          <p className="text-xs text-muted-foreground">Deixe em branco para usar a média mínima FIAP (6.0), ou informe uma nota específica</p>
        </div>

        <div className="flex gap-2">
          <Button onClick={handleCalculate} className="flex-1 bg-primary hover:bg-primary/90 shadow-lg shadow-primary/25">
            Calcular
          </Button>
          <Button variant="outline" onClick={handleReset} className="border-black/20 dark:border-white/20">
            Limpar
          </Button>
        </div>
      </CardContent>
